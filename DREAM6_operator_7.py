...
def run(
    ...
    # Closure integral (holonomie přes lock smyčky)
    ...
    # Residue compression (lokální→globální invariant)
    ...
    # ------------------ CLOSURE operators (část 2: po IPC - potřebuje 'a') ------------------
    # CLOSURE-FUSE cutoff (singulární řez) - needs clause phasors 'a'
    closure_diagnostics = {}
    if enable_closure_cut:
        # Aplikuj singular cut na clause phasors
        a_phases = np.angle(a) if hasattr(a, '__len__') else np.array([float(np.angle(a))])

        #a_regulated, fuse_diag = closure_fuse_cutoff(a_phases,
        #                                             threshold_percentile=95.0,
        #                                             epsilon=closure_epsilon)

        # ---- BRANCH RE-JOIN (post-singularity recombination) ----
        # Anchor: use the theta that actually drove the witness later.
        # At this point we don't know best_theta_used yet; use IPC theta as provisional.

        # IMPORTANT: actually apply rejoined phases back to clause phasors 'a'
        # keep amplitudes, replace phases

        a_regulated, fuse_diag = closure_fuse_cutoff(
            a_phases,
            threshold_percentile=95.0,
            epsilon=closure_epsilon,
            grad_cap=math.pi,  # nebo třeba 2.2 když chceš tvrdší “řez”
            mode="tanh"  # "clip" pro brutální strop
        )

        # 0) apply fuse-regulated phases to 'a' (keep amplitudes)
        ...
        anchor_theta = float(theta)

        phi = np.angle(a).copy()
        phi_rej, rejoin_diag = rejoin_branches_soft(
            phi,
            anchor=anchor_theta,
        ...
        # APPLY ONLY ON SINGULAR SET:
        phi[sing_mask] = phi_rej[sing_mask]
        a = np.abs(a) * np.exp(1j * phi)

        closure_diagnostics["rejoin"] = {
            **rejoin_diag,
            "applied_frac": float(np.mean(sing_mask)),
            "fuse": fuse_diag,
            "enabled": True,
        }
        print(f"CLOSURE-REJOIN(masked): applied_frac={closure_diagnostics['rejoin']['applied_frac']:.4f} "
              f"spread_before={rejoin_diag['spread_before']:.4f} spread_after={rejoin_diag['spread_after']:.4f}")
    ...
    print(
        f"CLOSURE-REJOIN: spread_before={rejoin_diag['spread_before']:.4f}  spread_after={rejoin_diag['spread_after']:.4f} "
        f"neg_before={rejoin_diag['frac_neg_before']:.3f} neg_after={rejoin_diag['frac_neg_after']:.3f}")

    # ------------------ CNF witness extraction (assignment projection) ------------------
    # ========== 3-BRANCH TESTING (MANIFEST MODE) ==========
    witness = {}
    if cnf_path:
        try:
            ...
            # Three candidate branches per manifest (3MILIONS.pdf, eq. 3):
            # Θ(Ψ) = {θ_U1, θ_Z2, θ_Z2 + π}
            ...
            for branch_name, th_test in candidates:
                ...
                print(f"  {branch_name:8s}: UNSAT = {unsat_test:3d}  "
            ...
            if enable_closure_cut:
                ...
                closure_diagnostics["rejoin_final"] = {
                    **re2,
                    "applied_frac": float(np.sum(sing_mask)) / float(len(sing_mask)),
                }
                print(
                    f"CLOSURE-REJOIN(masked): applied_frac={closure_diagnostics['rejoin_final']['applied_frac']:.4f} "
                    f"spread_before={re2['spread_before']:.4f} spread_after={re2['spread_after']:.4f} "
                    f"neg_before={re2['frac_neg_before']:.3f} neg_after={re2['frac_neg_after']:.3f}"
                )

            # after best_theta_used is chosen
            """if enable_closure_cut:
                phi2, re2 = rejoin_branches_soft(
                    np.angle(a),
                    anchor=float(best_theta_used),
                    eps=max(1e-12, float(closure_epsilon)),
                    temperature=0.12,
                ...
                a = np.abs(a) * np.exp(1j * phi2)
                closure_diagnostics["rejoin_final"] = re2"""
            ...
            print(
                f"  Phase spread: {best_diag['phase_spread']:.4f} rad  (concentration: {best_diag['phase_concentration']:.3f})")
            print(
                f"  Raw alignment: mean={best_diag['align_raw_mean']:+.3f}  |mean|={best_diag['align_raw_abs_mean']:.3f}")
            print(
                f"  Post auto-flip: mean={best_diag['align_post_mean']:+.3f}  |mean|={best_diag['align_post_abs_mean']:.3f}")
            print(f"  Fraction flipped: {100 * best_diag['frac_flipped']:.1f}%")
            print(
                f"  Bimodal signature (raw): ratio={best_diag['bimodal_ratio_raw']:.2f}  edge={best_diag['edge_mass_raw']:.3f}")
            ...
            witness = {
                ...
                "operator_diag": diag_results[best_branch_name],  # Selected branch diagnostics
                "operator_diag_all": diag_results,  # All branches for comparison
                "drive_stats": {
    ...
    cert_data = {
        ...
        "closure": closure_diagnostics,  # CLOSURE operators (holonomie, singulární řez)
