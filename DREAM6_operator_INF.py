...
def write_dimacs_model(path: str, assign: "np.ndarray") -> None:
    """Write a SAT model in a common DIMACS-like 'v ... 0' format.

    assign is a boolean array of length nvars, where assign[i] is value of variable (i+1).
    """
    n = int(len(assign))
    with open(path, "w", encoding="utf-8") as f:
        f.write("c model generated by DREAM6_operator\n")
        f.write("s SATISFIABLE\n")
        # write literals in chunks to keep lines reasonable
        chunk = 2000
        for start in range(0, n, chunk):
            lits = []
            for i in range(start, min(n, start + chunk)):
                v = i + 1
                lits.append(str(v if bool(assign[i]) else -v))
            f.write("v " + " ".join(lits) + " 0\n")


def unsat_clause_indices(clauses: list, assign: "np.ndarray") -> list:
    """Return 0-based indices of UNSAT clauses under the given assignment."""
    bad = []
    # clauses: list[list[int]] literals with sign
    for j, cl in enumerate(clauses):
        sat = False
        for lit in cl:
            v = abs(lit) - 1
...
def write_unsat_report(path: str, clauses: list, assign: "np.ndarray", limit: int = 5000) -> dict:
    """Write indices of UNSAT clauses (0-based) to a text file for independent validation.

    Returns a small summary dict.
    """
    idx = unsat_clause_indices(clauses, assign)
    shown = idx[:max(0, int(limit))] if limit is not None else idx
    with open(path, 'w', encoding='utf-8') as f:
        f.write(f"unsat_count={len(idx)}\n")
        for k in shown:
            f.write(str(k) + '\n')
    return {'unsat_count': len(idx), 'written': len(shown), 'path': path}
    ...
    for _ in range(int(sweeps)):
        for (i, j) in edges:
            ...
            for a, b in ranges:
                ...
                upd_i = (1.0 - eta_eff) * Zi_old + eta_eff * (sgn * Zj_old)
                upd_j = (1.0 - eta_eff) * Zj_old + eta_eff * (sgn * Zi_old)
...
def run(
    ...
    include_model_in_json: bool = False,
) -> Certificate:
    ...
    # ------------------ model / witness export ------------------
    # If we managed to compute a CNF witness assignment, optionally write it out for external validation.
    try:
        if cnf_path and isinstance(witness, dict) and ("error" not in witness):
            # try to retrieve assignment from local scope (set in the witness block)
            assign_model = locals().get("assign_wit", None)
            if assign_model is not None and hasattr(assign_model, "__len__"):
                if model_out:
                    write_dimacs_model(model_out, assign_model)
                    witness["model_out"] = str(model_out)
                if unsat_out:
                    rep = write_unsat_report(unsat_out, clauses, assign_model)
                    witness["unsat_out"] = str(unsat_out)
                    witness["unsat_recheck"] = rep
                # optionally embed the model in JSON (can be huge for large nvars)
                if bool(include_model_in_json):
                    # store as +/-1 list for compactness
                    witness["model"] = [1 if bool(b) else 0 for b in assign_model]
    except Exception as _e:
        # never fail the run due to export
        witness.setdefault("export_error", f"{type(_e).__name__}: {_e}")
...
def main() -> None:
    ...
    ap.add_argument("--model-out", type=str, default=None,
                   help="Write witness assignment (SAT model) to this file in DIMACS-like format.")
    ap.add_argument("--unsat-out", type=str, default=None,
                   help="Write UNSAT clause indices (0-based) under the witness assignment for validation.")
    ap.add_argument("--include-model-in-json", action="store_true",
                   help="Embed full assignment vector into the JSON witness (can be huge).")
    ...
    cert = run(
        ...
        include_model_in_json=args.include_model_in_json,
    )
